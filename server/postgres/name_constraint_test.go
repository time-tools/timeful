...
const (
	eventNameConstraintMigration = "20260913000002_events_name_length.sql"
	eventNameValidationMigration = "20260914000000_validate_events_name_length.sql"
)
...
WHERE conrelid = 'events'::regclass AND conname = 'events_name_length'`).Scan(&validated, &definition); err != nil {
		t.Fatalf("read events_name_length: %v", err)
	}
	if !validated {
		t.Fatal("expected events_name_length to be validated")
	}
	...
	for _, testCase := range accepted {
		t.Run("accepts "+testCase.name, func(t *testing.T) {
			if _, err := tx.Exec(ctx, `INSERT INTO events (short_id, name, type) VALUES ($1, $2, 'signup')`, signupTestShortID(t), testCase.value); err != nil {
	...
	for _, testCase := range rejected {
		t.Run("rejects "+testCase.name, func(t *testing.T) {
			err := expectSavepointError(t, ctx, tx, func() error {
				_, err := tx.Exec(ctx, `INSERT INTO events (short_id, name, type) VALUES ($1, $2, 'signup')`, signupTestShortID(t), testCase.value)
				return err
			})
			var postgresError *pgconn.PgError
			if !errors.As(err, &postgresError) || postgresError.Code != "23514" || postgresError.ConstraintName != "events_name_length" {
				t.Fatalf("expected events_name_length check violation, got %v", err)
			}
		})
	}
}
...
func TestPostgresEventsNameLengthConstraintDown(t *testing.T) {
	...
	WHERE conrelid = 'events'::regclass AND conname = 'events_name_length')`).Scan(&exists); err != nil {
		t.Fatalf("read events_name_length existence: %v", err)
	}
	if exists {
		t.Fatal("expected the Down migration to drop events_name_length")
	}
	if _, err := tx.Exec(ctx, `INSERT INTO events (short_id, name, type) VALUES ($1, $2, 'signup')`, signupTestShortID(t), strings.Repeat("a", 101)); err != nil {
...
WHERE conrelid = 'events'::regclass AND conname = 'events_name_length'`).Scan(&validated); err != nil {
		t.Fatalf("read events_name_length: %v", err)
	}
	if validated {
